...
const HistoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        if (user && user.role !== 'admin') {
            dataService.getHistoryForUser(user.email).then(userHistory => {
                if (isMounted) {
    ...
    const addOrUpdateHistory = useCallback(async (entry: AnamnesisFormData) => {
        if (!user) return;
...
const VideosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        let isMounted = true;
        dataService.getVideos().then(initialVideos => {
            if (isMounted) {
    ...
    const addVideo = useCallback(async (video: Video) => {
       try {
            const newVideos = await dataService.saveVideo(video);
            setVideos(newVideos);
        } catch (e) {
